...
После разбора файла берутся множества уникальных **базовых ER** и уникальных
**добавочных кодов**. Каждый ER комбинируется с каждым добавочным кодом этого
файла, включая `0000` («без кабельных вводов») и `0001`.

Имена и номера строк сохраняются из конкретного файла, а для глобальной
дедупликации используются канонические `id` базы данных.
...
- **Варианты:** `variantPairsProcessed / variantPairsTotal`
  (`uniqueBases × uniqueAdds` для файла).  
